...
def main():
    ...
    with open('../../data/mediation_tst_shifted.pkl', 'rb') as fp:
        data = pkl.load(fp)
    df = pd.DataFrame({'X': data['T'].astype(int), 'Y': data['Y'].squeeze(), 'm': data['Z'][:,0]})
    mediator = data['M']
    ...
    for runs in range(num_runs):
        ...
        params_df,z_final = simulate_mediation(df,mediator,output_df,model,batch_size,epochs,
                                      False,runs,num_iters,patience,verbose)
    params_df.to_csv(os.path.join(result_path,'tst_exp_with_covariate_svr_LSEM.csv'))
    
    res = pd.read_csv('./results/csv_files/tst_exp_with_covariate_svr_LSEM.csv')
    ...
    with open('./results/tst_exp_with_covariate_svr_LSEM.pkl', 'wb') as fp:
        pkl.dump(res, fp)
