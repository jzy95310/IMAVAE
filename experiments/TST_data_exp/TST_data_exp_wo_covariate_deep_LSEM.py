...
def create_deep_dense_network(hidden_dim):
    # Create a three-layer dense network
    model = Sequential([
        Dense(hidden_dim, activation=partial(tf.nn.leaky_relu, alpha=0.01), name='dense1'), 
        Dense(hidden_dim, activation=partial(tf.nn.leaky_relu, alpha=0.01), name='dense2'), 
        Dense(hidden_dim, activation=partial(tf.nn.leaky_relu, alpha=0.01), name='dense3'), 
...
def main():
    ...
    for runs in range(num_runs):
        ...
        # create a model
        model = create_deep_dense_network(hidden_dim=128)
    ...
    params_df.to_csv(os.path.join(result_path,'tst_exp_wo_covariate_deep_LSEM.csv'))
    
    res = pd.read_csv('./results/csv_files/tst_exp_wo_covariate_deep_LSEM.csv')
    ...
    beta_std = np.std(np.array(res['beta.{}'.format(num_iters-1)][1:]).astype(float))
    ...
    with open('./results/tst_exp_wo_covariate_deep_LSEM.pkl', 'wb') as fp:
        pkl.dump(res, fp)
